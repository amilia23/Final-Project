...
<body>
 <div>
    ...
    <div class="container">
    <div class="box-login">
        <div class="header-box">Student Login</div>
        <br><br>
        <center>
...
<?php 
    ...
    if(isset($_POST['submit'])){
        ...
        $check = mysqli_query($mysqli, "SELECT * FROM student WHERE email = '".$email."'AND password = '".$password."'");
        ...
        if(mysqli_num_rows($check) > 0){
            ...
            if($role_pelogin == 'student'){
                header('location:home.php');
            }
            else{
                echo  "<script type=\"text/javascript\">window.alert('Error: Wrong email or password'); window.location.href = 'studentlogin.php'; </script>";
            }  
        }
    else{
        echo  "<script type=\"text/javascript\">window.alert('Error: Wrong email or password'); window.location.href = 'studentlogin.php'; </script>";
